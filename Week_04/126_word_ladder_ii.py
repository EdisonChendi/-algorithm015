        ...
        while q and not stop:
            ...
            while q:
                ...
                if not stop:
                    for i in range(L):
                        pat = w[:i] + "*" + w[i+1:]
                        for word in patterns[pat]:
                            if word not in visited:
                                cur_round_visited.add(word)
                                next_q.append((word, ladder+[word, ]))
            visited.update(cur_round_visited)
            q = next_q
        return res
...
class TestSolution(unittest.TestCase):
    ...
    #         ["hit", "hot", "lot", "log", "cog"]
    #     ]
    #     self.assertCountEqual(sol.findLadders(
